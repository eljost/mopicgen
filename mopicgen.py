...
def gen_jmol_input(fn, orient, mos, ifx):
    """Jmol MO indices are always 1-based but the 'mos' argument is always
    0-based. So we use two lists: One list holding the MO indices for Jmol
    (1-based, 'jmol_mos') and one list holding the MO indices for the
    generation of the filenames (0-based, 'mos')."""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fracmos", action="store_true", help="Consider all"
                        " MOs with fractional occupation (different from "
                        "0 and 2), e.g. MOs in an active space.")
    ...
    if is_orca or args.fracmos:
        # With .molden files from ORCA or automatically determined
        # MOs the MO-indexing is 0 based.
        # To mimic 1-based user input we add 1 to all MO indices.
